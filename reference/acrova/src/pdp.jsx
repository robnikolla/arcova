// Product Detail Page — AW-72 Aluminium Window System
function PDP({ setPage, onQuote }) {
  ...
  return (
    <main>
      <Breadcrumb setPage={setPage} />
      <ProductHero onQuote={onQuote} viewTab={viewTab} setViewTab={setViewTab} />
      <ConfigStrip
        selectedOpening={selectedOpening} setSelectedOpening={setSelectedOpening}
        selectedGlazing={selectedGlazing} setSelectedGlazing={setSelectedGlazing}
      ...
      <TechnicalSpecs />
      <Downloads />
      <RelatedSystems setPage={setPage} />
    </main>
  );
}

/* ---------- BREADCRUMB ---------- */
function Breadcrumb({ setPage }) {
  return (
    <div style={{ borderBottom: "1px solid var(--line)" }}>
      <div className="container" style={{ padding: "16px var(--pad-x)", display: "flex", gap: 12, fontFamily: "var(--f-mono)", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--muted)" }}>
        <button onClick={() => setPage("home")}>Index</button>
        <span>/</span>
        <button onClick={() => setPage("home")}>Systems</button>
        <span>/</span>
        <span>Aluminium Windows</span>
        <span>/</span>
        <span style={{ color: "var(--ink)" }}>AW-72</span>
      </div>
    </div>
  );
}
...
/* ---------- PRODUCT HERO ---------- */
function ProductHero({ onQuote, viewTab, setViewTab }) {
  const tabs = [
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 56, alignItems: "start" }}>
          ...
          <div style={{ position: "sticky", top: 96 }}>
            ...
            {viewTab === "elevation" ? (
              ...
              <Placeholder aspect="4/5" label={tabs.find(t => t.k === viewTab).l.toUpperCase()} caption={captions[viewTab]} />
            )}
          ...
          <div style={{ paddingTop: 8 }}>
            ...
            <h1 style={{ fontSize: "clamp(44px, 5vw, 72px)", marginBottom: 24, textWrap: "balance" }}>
              Thermal-break<br />aluminium, 72 mm.
            </h1>

            <p style={{ fontSize: 19, color: "var(--ink-2)", marginBottom: 32, maxWidth: 480, lineHeight: 1.5 }}>
              A flagship European-profile window for contemporary facades. Three-chamber aluminium, polyamide thermal bridge, concealed EPDM seals. Engineered for commercial and high-spec residential.
            </p>
            ...
            <div style={{ border: "1px solid var(--line-2)", padding: "20px 24px", marginBottom: 32, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 24 }}>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                  <span style={{ fontSize: 20, fontWeight: 500 }}>aluplast · Salamander</span>
                </div>
...
/* ---------- Elevation drawing (technical SVG) ---------- */
function ElevationDrawing() {
  return (
    <div style={{ background: "var(--bg-2)", padding: 40, aspectRatio: "4/5", position: "relative", border: "1px solid var(--line)" }}>
      <Corners />
      <svg viewBox="0 0 400 500" style={{ width: "100%", height: "100%", display: "block" }}>
        ...
        {/* Tilt-turn indicators (diagonals) */}
...
/* ---------- CONFIG STRIP ---------- */
function ConfigStrip({ selectedOpening, setSelectedOpening, selectedGlazing, setSelectedGlazing, selectedFinish, setSelectedFinish, onQuote }) {
  const openings = [
  ...
  return (
    <section style={{ borderBottom: "1px solid var(--line)", background: "var(--bg-2)" }}>
      <div className="container" style={{ padding: "80px var(--pad-x)" }}>
        ...
        {/* Opening types */}
        <ConfigRow label="Opening type" idx="01">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 0, border: "1px solid var(--line-2)" }}>
            {openings.map((o, i) => (
              <button
                key={o.k}
                onClick={() => setSelectedOpening(o.k)}
                style={{
                  padding: "24px 16px",
                  borderRight: i < openings.length - 1 ? "1px solid var(--line-2)" : "none",
              ...
              >
                <OpeningIcon type={o.k} active={selectedOpening === o.k} />
        ...
        {/* Glazing */}
        <ConfigRow label="Glazing" idx="02">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, border: "1px solid var(--line-2)" }}>
            {glazings.map((g, i) => (
              <button
                key={g.k}
                onClick={() => setSelectedGlazing(g.k)}
                style={{
                  padding: "20px",
                  borderRight: i < glazings.length - 1 ? "1px solid var(--line-2)" : "none",
        ...
        {/* Finish */}
        <ConfigRow label="Finish" idx="03">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 0, border: "1px solid var(--line-2)" }}>
            {finishes.map((f, i) => (
              <button
                key={f.k}
                onClick={() => setSelectedFinish(f.k)}
                style={{
                  padding: "16px",
                  borderRight: i < finishes.length - 1 ? "1px solid var(--line-2)" : "none",
              ...
              >
                <div style={{ width: 28, height: 28, background: f.sw, border: "1px solid rgba(0,0,0,0.15)" }} />
...
function ConfigRow({ label, idx, children }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 40, padding: "32px 0", borderBottom: "1px solid var(--line-2)", alignItems: "start" }}>
      <div>
        <div style={{ fontFamily: "var(--f-mono)", fontSize: 10, color: "var(--muted)", letterSpacing: "0.14em", marginBottom: 6 }}>STEP {idx}</div>
        <div style={{ fontSize: 20, fontWeight: 500 }}>{label}</div>
      </div>
      <div>{children}</div>
    </div>
  );
...
function OpeningIcon({ type, active }) {
...
function PerformanceMatrix() {
  ...
  const cols = ["AW-72 · Std", "AW-72 · Dbl", "AW-72 · Acoustic", "AW-72 · Security"];
        ...
        <div style={{ border: "1px solid var(--line-2)", overflow: "hidden" }}>
          ...
          {data.map((d, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr repeat(4, 1fr)", borderTop: i > 0 ? "1px solid var(--line)" : "none", background: i % 2 === 0 ? "var(--bg)" : "var(--bg-2)" }}>
              ...
              {d.vals.map((v, j) => (
                <div key={j} style={{ padding: "20px", fontFamily: "var(--f-mono)", fontSize: 15, fontWeight: 500, borderLeft: "1px solid var(--line)", background: j === 0 ? "color-mix(in oklab, var(--accent) 8%, transparent)" : "transparent" }}>
                  {v}
                </div>
              ))}
            </div>
      ...
      <div className="container" style={{ padding: "120px var(--pad-x)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          <div>
            ...
            <h2 style={{ color: "var(--bg)", textWrap: "balance", marginBottom: 24 }}>
              Three chambers.<br />
              One weather seal.
            </h2>
            <p style={{ fontSize: 17, color: "rgba(255,255,255,0.7)", lineHeight: 1.55, marginBottom: 40, maxWidth: 480 }}>
              The AW-72 profile runs a 24 mm polyamide thermal break between
              extruded aluminium shells. Co-extruded EPDM gaskets seat against the
              sash, eliminating the secondary brush seal common to cheaper systems.
            </p>
...
/* ---------- TECHNICAL SPECS (expanded table) ---------- */
    ...
    <section style={{ borderBottom: "1px solid var(--line)", background: "var(--bg-2)", padding: "100px 0" }}>
      <div className="container">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 0, border: "1px solid var(--line-2)" }}>
          {files.map((f, i) => (
            <a href="#" key={f.t} style={{
              padding: "24px 28px",
              borderBottom: i < files.length - 2 ? "1px solid var(--line-2)" : "none",
...
/* ---------- RELATED ---------- */
function RelatedSystems({ setPage }) {
  const r = [
    { tag: "AW-85", t: "Passive-House Aluminium", u: "0.68", img: IMGS.facade_metallic },
    { tag: "PW-88", t: "Thermal PVC Alternative", u: "0.74", img: IMGS.arch_b },
    { tag: "CW-60", t: "Matching Curtain Wall", u: "1.1", img: IMGS.facade_geometric },
  ];
  return (
    <section style={{ padding: "100px 0" }}>
      <div className="container">
        <SectionHeader
          eyebrow="Compatible systems"
          title="Specified together."
        />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 32 }}>
          {r.map((x) => (
            <button key={x.tag} onClick={() => setPage("pdp")} className="prod-card" style={{ textAlign: "left", padding: 0 }}>
              <Placeholder aspect="4/5" src={x.img} label={x.tag} />
              <div style={{ padding: "20px 0 0", display: "flex", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontFamily: "var(--f-mono)", fontSize: 11, color: "var(--muted)", letterSpacing: "0.14em", marginBottom: 6, textTransform: "uppercase" }}>{x.tag}</div>
