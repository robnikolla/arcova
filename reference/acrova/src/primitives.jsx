...
function Arrow({ dir = "right", size = 14 }) {
  ...
  return (
    <svg width={size} height={size * 0.72} viewBox="0 0 14 10" fill="none" style={{ transform: `rotate(${rot}deg)` }}>
...
function Logo({ size = 28, showWord = true, color }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, color: c }}>
      ...
      {showWord && (
        <span style={{ fontFamily: "var(--f-display)", fontWeight: 600, fontSize: 18, letterSpacing: "0.04em", textTransform: "uppercase" }}>ACROVA</span>
      )}
    </div>
  );
}
...
function Placeholder({ label = "IMAGE", caption, aspect = "4/3", style = {}, src, children }) {
  const [imgFailed, setImgFailed] = useState(false);
  if (src && !imgFailed) {
    return (
      <div style={{ position: "relative", aspectRatio: aspect, width: "100%", overflow: "hidden", background: "var(--placeholder)", ...style }}>
        <img
          src={src}
          alt={label}
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 40, marginBottom: 40, flexWrap: "wrap" }}>
      <div style={{ maxWidth: 820 }}>
        <div className="eyebrow" style={{ marginBottom: 20 }}>
          ...
          {eyebrow && <><span className="dot" /><span>{eyebrow}</span></>}
        </div>
        <h2 style={{ textWrap: "balance" }}>{title}</h2>
        {subtitle && <p style={{ marginTop: 20, color: "var(--muted)", fontSize: 17, maxWidth: 640, textWrap: "pretty" }}>{subtitle}</p>}
      </div>
      {rightSlot}
    </div>
  );
...
function Corners() {
  const s = { position: "absolute", width: 10, height: 10, border: "1px solid var(--line-2)" };
  return (
    <>
...
Object.assign(window, { Arrow, Logo, LogoMark, Placeholder, SpecRow, SectionHeader, Corners, IMGS });
